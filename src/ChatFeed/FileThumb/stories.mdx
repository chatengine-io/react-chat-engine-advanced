...
export const Template = ({ children, isLoading, attachment, style }) => (
  <FileThumb attachment={attachment} isLoading={isLoading} style={style}>
    {children}
  </FileThumb>
);
...
The component requires an `attachment` prop (of type ``) to display the correct file info.

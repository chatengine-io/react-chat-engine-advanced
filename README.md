...
4. Import the `ChatWindow`, `UserSocket`, and `useUserHooks`

5. Declare `const props = useUserHooks(projectId, myUsername, mySecret);` and pass the returning `props` object into `ChatWindow` and `UserSocket`.
